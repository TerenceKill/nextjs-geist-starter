...
void display_zeile2_formatieren(char* zeile, const SensorDaten* daten) {
    ...
    if (!daten->gueltig) {
        strcpy(status_text, "SENSOR-FEHLER! Daten ungueltig");
    }
        ...
        if (offen_dauer > DOOR_OPEN_THRESHOLD) {
            snprintf(status_text, sizeof(status_text), 
                    "TUER ZU LANGE OFFEN! %ld Sek", offen_dauer);
        } else {
            snprintf(status_text, sizeof(status_text), 
                    "Tuer offen seit %ld Sekunden", offen_dauer);
        }
    }
        ...
        char temp_balken[21]; // 20 Zeichen + Null-Terminator
        fortschrittsbalken_erstellen(daten->temperatur, -5.0f, 15.0f, 20, temp_balken);
        snprintf(status_text, sizeof(status_text), 
