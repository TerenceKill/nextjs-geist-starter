...
void display_zeile2_formatieren(char* zeile, const SensorDaten* daten) {
    char status_text[DISPLAY_COLS + 1] = {0};
    
    // Status-Meldungen basierend auf Sensor-Werten
    if (!daten->gueltig) {
        strncpy(status_text, "SENSOR-FEHLER! Daten ungueltig", DISPLAY_COLS);
    }
    else if (daten->temperatur > MAX_TEMP_THRESHOLD) {
        snprintf(status_text, DISPLAY_COLS + 1, 
                "TEMP ZU HOCH! %.1f%c > %.1f%c", 
                daten->temperatur, GRAD_ZEICHEN, 
                MAX_TEMP_THRESHOLD, GRAD_ZEICHEN);
    }
    else if (daten->temperatur < MIN_TEMP_THRESHOLD) {
        snprintf(status_text, DISPLAY_COLS + 1, 
                "TEMP ZU NIEDRIG! %.1f%c < %.1f%c", 
                daten->temperatur, GRAD_ZEICHEN, 
        ...
        if (offen_dauer > DOOR_OPEN_THRESHOLD) {
            snprintf(status_text, DISPLAY_COLS + 1, 
                    "TUER ZU LANGE OFFEN! %ld s", offen_dauer);
        } else {
            snprintf(status_text, DISPLAY_COLS + 1, 
                    "Tuer offen seit %ld s", offen_dauer);
        }
    }
    else if (daten->energie_verbrauch > MAX_ENERGY_THRESHOLD) {
        snprintf(status_text, DISPLAY_COLS + 1, 
                "ENERGIE ZU HOCH! %.0fW > %.0fW", 
                daten->energie_verbrauch, MAX_ENERGY_THRESHOLD);
    }
    else {
        // Normaler Betrieb - zeige Temperatur-Fortschrittsbalken
        char temp_balken[16]; // Kürzer für bessere Anpassung
        fortschrittsbalken_erstellen(daten->temperatur, -5.0f, 15.0f, 15, temp_balken);
        snprintf(status_text, DISPLAY_COLS + 1, 
                "Status: OK [%s]", temp_balken);
    }
    
    // Text in Zeile kopieren und auffüllen
    strncpy(zeile, status_text, DISPLAY_COLS);
